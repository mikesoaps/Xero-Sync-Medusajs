...
export class Migration20260330051958 extends Migration {
  override async up(): Promise<void> {
    ...
    this.addSql(`create table if not exists "xero_item_link" ("id" text not null, "medusa_product_id" text not null, "xero_item_id" text null, "xero_update_token" text null, "tenant_id" text null, "last_synced_hash" text null, "last_direction" text null, "last_synced_at" timestamptz null, "metadata" jsonb null, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "xero_item_link_pkey" primary key ("id"));`);
    this.addSql(`CREATE INDEX IF NOT EXISTS "IDX_xero_item_link_deleted_at" ON "xero_item_link" ("deleted_at") WHERE deleted_at IS NULL;`);
  }
  ...
  override async down(): Promise<void> {
    ...
    this.addSql(`drop table if exists "xero_item_link" cascade;`);
  }
}
